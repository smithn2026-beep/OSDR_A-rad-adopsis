...
deg_file <- "C:/Users/smith/OneDrive - Florida Institute of Technology/summer 2026/OSDR/OSDR_A-rad-adopsis/csv Results/DEG_OSD498_510_radiation_effect.csv"
...
for (bo in organ_order) {
  ...
  volcano_data <- rbind(
    data.frame(constitutive[, c("log2FoldChange", "neg_log10_padj",
                                "deg_flag", "regulation")],
               category = "constitutive"),
    data.frame(organ_genes[, c("log2FoldChange", "neg_log10_padj",
                               "deg_flag", "regulation")],
               category = bo)
  )
